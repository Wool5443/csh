...
INLINE const char* getCompiler_()
{
    ERROR_CHECKING();
...
INLINE ErrorCode gatherCompilerArgsImpl_(const Str argType, CompilerArgsList* args, String sourceFile)
{
    ERROR_CHECKING();
...
INLINE ResultCompilerArgsList GatherCompilerArgs(const Str sourcePath, const Str executablePath)
{
    ...
    {
        CHECK_ERROR(gatherCompilerArgsImpl_(StrCtor(ARG_TYPES[i]), &args, sourceFile));
    }
...
INLINE ResultArgsList GatherCommandLineArgs(const char outputPath[static 1], const int argc, const char* args[])
{
    ERROR_CHECKING();
